...
export default function LandingClassic() {
  useEffect(() => {
    ...
    document.querySelectorAll(".experience-card, .work-item").forEach((el) => {
...
const markup = `
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .rotating-text{display:inline-block;background:linear-gradient(135deg,#d4af37 0%,#f4d03f 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;animation:slideUp 8s infinite}
    ...
    .resume-btn{padding:1rem 2.5rem;background:linear-gradient(135deg,#d4af37 0%,#f4d03f 100%);color:#0a0a0a;font-size:1.1rem;font-weight:600;border:none;border-radius:50px;cursor:pointer;transition:all .3s;text-decoration:none;display:inline-block}
    .resume-btn:hover{transform:translateY(-3px);box-shadow:0 10px 30px rgba(212,175,55,.3)}
    ...
    .floating-element{position:absolute;width:300px;height:300px;border-radius:50%;background:radial-gradient(circle,rgba(212,175,55,.1) 0%,transparent 70%);pointer-events:none;animation:float 6s ease-in-out infinite}
    ...
    .skill-item{font-size:1.8rem;font-weight:600;padding:1.5rem 0;border-bottom:1px solid rgba(255,255,255,.1);transition:all .3s;cursor:pointer;position:relative;overflow:hidden}
    .skill-item::before{content:"";position:absolute;left:0;bottom:0;width:0;height:2px;background:linear-gradient(90deg,#d4af37,#f4d03f);transition:width .5s ease}
    .skill-item:hover::before{width:100%}
    .skill-item:hover{padding-left:2rem;color:#d4af37;border-color:#d4af37}
    ...
    .experience-card{padding:3rem;background:rgba(255,255,255,.02);border-radius:20px;border:1px solid rgba(255,255,255,.05);transition:all .5s cubic-bezier(.23,1,.32,1);cursor:pointer;position:relative;overflow:hidden}
    .experience-card::after{content:"";position:absolute;top:0;left:-100%;width:100%;height:100%;background:linear-gradient(90deg,transparent,rgba(212,175,55,.1),transparent);transition:left .7s}
    .experience-card:hover::after{left:100%}
    .experience-card:hover{background:rgba(255,255,255,.04);border-color:rgba(212,175,55,.3);transform:translateY(-5px)}
    ...
    .work-item{background:linear-gradient(135deg,#1e1e1e 0%,#2a2a2a 100%);border-radius:20px;padding:3rem;transition:all .5s cubic-bezier(.23,1,.32,1);cursor:pointer;position:relative;overflow:hidden;border:1px solid rgba(255,255,255,.05)}
    .work-item::before{content:"";position:absolute;top:0;left:0;right:0;bottom:0;background:linear-gradient(135deg,#d4af37 0%,#f4d03f 100%);opacity:0;transition:opacity .5s}
    .work-item:hover{transform:translateY(-10px) scale(1.02);box-shadow:0 20px 40px rgba(212,175,55,.2)}
    ...
    .partner-logo{height:70px;background:rgba(255,255,255,.03);border-radius:15px;display:flex;align-items:center;justify-content:center;font-size:1.1rem;font-weight:600;opacity:.6;transition:all .4s cubic-bezier(.23,1,.32,1);border:1px solid rgba(255,255,255,.05);position:relative;overflow:hidden}
    .partner-logo::before{content:"";position:absolute;top:50%;left:50%;width:0;height:0;border-radius:50%;background:rgba(212,175,55,.1);transform:translate(-50%,-50%);transition:width .6s,height .6s}
    .partner-logo:hover::before{width:200%;height:200%}
    .partner-logo:hover{opacity:1;background:rgba(255,255,255,.08);border-color:rgba(212,175,55,.3);transform:scale(1.05)}
